...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "out-connect-stations": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == "train_arrival_events": # Set the conditional to the arrival topic
